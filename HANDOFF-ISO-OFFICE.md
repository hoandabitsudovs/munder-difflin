...
- **Frontera real que queda: los 35 personajes son sprites top-down en un mundo iso.** El redibujo en ángulo iso (proporciones/sombreado/8 direcciones) es el gap de fidelidad más grande y es una **tarea dedicada de varias sesiones**, no un pase suelto. Empezar por planear alcance (¿todos los personajes o un subset? ¿quién dibuja?) antes de tocar `portraitArt.ts`/`CharacterSprite.ts`. El resto son detalles menores (props de pared laterales, animación de pantallas, ventanas en más muros).
