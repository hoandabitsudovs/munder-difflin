...
**El usuario decidió NO comprar packs.** Quiere usar las referencias iso (LimeZu, Isometric Interiors de pixel_Salvaje, Agent-Pixels, etc.) **solo como inspiración** para crear arte propio "mejor o casi igual, pero sin licencia" — es decir, seguir en procedural (opción C) pero con pases de **dirección de arte** que se acerquen al look de la referencia, todo original (nada de copiar píxeles ni código). **No volver a proponer comprar packs salvo que el usuario lo pida.**

**Pases de dirección de arte ya hechos (10 sept)** — el ENTORNO quedó muy pulido:
- Alfombras de área por sala (`ccf49fa7`), escritorios con monitor encendido + materiales de piso/pared (`d9f1b83b`), sombra de contacto bajo personajes (`1b1756ce`), sillas en escritorios de deptos (`b6b6df64`), paredes tintadas por depto + chair-rail (`37e7f581`), ventanas con cielo en el perímetro (`0d60f0e9`). (Antes: piezas temáticas, iluminación, colisión, esquinas, etc.)
- ~~**Frontera: los 35 personajes son sprites top-down en un mundo iso.**~~ **✅ HECHO (12 sept, commit `ea19af47`).** Se portó el estilo iso chibi aprobado (`prototypes/isometric/lib/char.mjs`) al motor: `src/scene/office/isoCharArt.ts` (nuevo) dibuja los 35 en chibi iso con ciclo de caminata + vista de espalda; `cast.ts` lo usa vía `isoSceneFrameBufs` (dims 20×33). El generador top-down (`portraitArt.sceneFrameBufs`) queda como fallback. **Los retratos de las tarjetas siguen top-down** (podrían migrarse a iso para consistencia, pendiente menor). Detalles finos posibles: pulir el ciclo de caminata (hoy es un leve alzado de pie), pose sentado dedicada, más direcciones. El resto son detalles menores (props de pared, animación de pantallas).
