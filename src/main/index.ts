...
// ─── Shareable hires: agenticos:// deep link + file import ──────────────
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('agenticos', process.execPath, [resolve(process.argv[1])]);
  }
} else {
  app.setAsDefaultProtocolClient('agenticos');
}
...
if (!gotInstanceLock) {
  ...
  app.on('second-instance', (_evt, argv) => {
    ...
    const link = argv.find((a) => a.startsWith('agenticos://'));
...
function createWindow(opts: { floor?: boolean } = {}): BrowserWindow {
  ...
  const win = new BrowserWindow({
    ...
    title: isFloor ? 'Agentic OS IMAJU — Floor' : 'Agentic OS IMAJU',
...
app.whenReady().then(() => {
  ...
  const startupHireLink = process.argv.find((a) => a.startsWith('agenticos://'));
