  ...
  switch (status?.state) {
    ...
    case 'downloaded':
      return {
        ...
        detail: `Restart Agentic OS IMAJU to finish updating from v${v}.`,
...
export function manualInstallSteps(platform: string): { os: string; steps: string[] } {
  if (platform === 'darwin') {
    return {
      os: 'macOS',
      steps: [
        'Open the .dmg and drag Agentic OS IMAJU onto Applications. Choose Replace when asked.',
  ...
  if (platform === 'win32') {
    return {
      ...
      steps: [
        ...
        'Open Agentic OS IMAJU again and pick the same project.'
      ]
    };
  }
  return {
