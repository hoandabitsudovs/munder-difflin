 ...
 * `agenticos://hire?src=<https-url>` deep link or an in-app file picker.
 *
...
export const HIRE_SPEC_V1 = 'agentic-os-imaju/hire@1';
...
export interface HireManifest {
  /** Spec tag; exactly `agentic-os-imaju/hire@1` for this version. */
...
/** Parse a `agenticos://hire?src=<https-url>` deep link. Returns the https
...
export function parseHireDeepLink(link: string): string | null {
  ...
  if (u.protocol !== 'agenticos:') return null;
  // Both agenticos://hire?src= (host) and agenticos:hire?src= (path).
